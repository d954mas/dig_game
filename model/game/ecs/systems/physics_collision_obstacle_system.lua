local BaseSystem = require "model.game.ecs.systems.physics_collision_base_system"
...
function System:is_handle_collision(info)
	...
	return (e1.cell or e2.cell) or (e1.obstacle or e2.obstacle)
end

---@param info NativePhysicsCollisionInfo
function System:handle_collision(info)
	local e1, e2 = info.body1:get_user_data(), info.body2:get_user_data()
	local cell_first = (e1.cell or e1.obstacle) and true or false;
	local manifold = info.manifolds[1]
	local point = manifold.points[1]
	self:handle_geometry(cell_first and e2 or e1, vmath.vector3(-point.normal.x, point.normal.y, point.normal.z), point.depth)
end
---@param e Entity
