...
function Curve:initialize(config)
	...
	for _, point in ipairs(config.points) do
		table.insert(self.points, vmath.vector3(point))
	end
...
function Curve:_calculate()
	...
	for i = 2, pl, 1 do
		...
		for t = 0, self.segments do
			...
			table.insert(res, vmath.vector3(x, y, 0))
		end
	end
	...
	for i = 1, #self.points_calculated - 1 do
		...
		table.insert(self.segments, { len = v_len, p1 = start_point, p2 = end_point, vector = v, curve_len_star = start_len, curve_len_finish = self.len })
	end
end

function Curve:point_interpolated_get(a)
	a = COMMON.LUME.clamp(a, 0, 1)
	if (a == 1) then return vmath.vector3(self.points[#self.points]) end
	...
	for _, segment in ipairs(self.segments) do
		if (dist < segment.curve_len_finish) then
			...
			return segment.p1 + segment.vector * point_a
		end
	end
end
